...
class NewContactPage extends Component {
	...
	saveContact() {
		if (this.isAllowed()) {
			...
			const users = CollectionUtils.getUserForRoom(this.state.owner,
				[{
					title: title,
					email: email
				}]);
			getStoredDataFromKey(EMAIL).
				then((mail) => {
					...
					let chat = CollectionUtils.createChat(title.trim(), 'No new message',
						false, Type.PERSONAL, room, email, 0, null, null, users);
	...
	render() {
		return (
			<View style={styles.base}>
				<Toolbar centerElement='New contact'
